require "rails_helper"

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/meals", type: :request do
  let(:user) { create(:user) }
  let(:workspace) { create(:workspace) }
  let!(:workspace_user) { create(:workspace_user, user: user, workspace: workspace) }

  before {
    login_as(user)
    patch "/workspaces/#{workspace.id}/switch"
  }

  describe "GET /index" do
    it "renders a successful response" do
      workspace.meals.create!(name: "MyString", description: "MyText", serving_for: 2, workspace: workspace, created_by: user)
      get meals_url
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      meal = workspace.meals.create!(name: "Tuna Salad", description: "MyText", serving_for: 2, workspace: workspace, created_by: user)
      get meal_url(meal)
      expect(response).to be_successful
    end
  end

  describe "GET /new" do
    it "renders a successful response" do
      get new_meal_url
      expect(response).to be_successful
    end
  end

  describe "GET /edit" do
    it "renders a successful response" do
      meal = workspace.meals.create!(name: "Tuna Salad", description: "MyText", serving_for: 2, workspace: workspace, created_by: user)
      get edit_meal_url(meal)
      expect(response).to be_successful
    end
  end

  describe "POST /create" do
    context "with valid parameters" do
      it "creates a new Meal" do
        expect {
          post meals_url, params: {meal: {name: "Tuna Salad", description: "MyText", serving_for: 2}}
        }.to change(Meal, :count).by(1)
      end

      it "redirects to the created meal" do
        post meals_url, params: {meal: {name: "Tuna Salad", description: "MyText", serving_for: 2}}
        expect(response).to redirect_to(meal_url(Meal.last))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Meal" do
        expect {
          post meals_url, params: {meal: {name: ""}}
        }.to change(Meal, :count).by(0)
      end

      it "renders a response with 422 status (i.e. to display the 'new' template)" do
        post meals_url, params: {meal: {name: ""}}
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) {
        {name: "New Testing"}
      }

      it "updates the requested meal" do
        meal = workspace.meals.create!(name: "Tuna Salad", description: "MyText", serving_for: 2, workspace: workspace, created_by: user)
        patch meal_url(meal), params: {meal: new_attributes}
        meal.reload
        expect(meal.name).to eq("New Testing")
      end

      it "redirects to the meal" do
        meal = workspace.meals.create!(name: "Tuna Salad", description: "MyText", serving_for: 2, workspace: workspace, created_by: user)
        patch meal_url(meal), params: {meal: new_attributes}
        meal.reload
        expect(response).to redirect_to(meal_url(meal))
      end
    end

    context "with invalid parameters" do
      it "renders a response with 422 status (i.e. to display the 'edit' template)" do
        meal = workspace.meals.create!(name: "Tuna Salad", description: "MyText", serving_for: 2, workspace: workspace, created_by: user)
        patch meal_url(meal), params: {meal: {name: ""}}
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested meal" do
      meal = workspace.meals.create!(name: "Tuna Salad", description: "MyText", serving_for: 2, workspace: workspace, created_by: user)
      expect {
        delete meal_url(meal)
      }.to change(workspace.meals, :count).by(-1)
    end

    it "redirects to the meals list" do
      meal = workspace.meals.create!(name: "Tuna Salad", description: "MyText", serving_for: 2, workspace: workspace, created_by: user)
      delete meal_url(meal)
      expect(response).to redirect_to(meals_url)
    end
  end
end
