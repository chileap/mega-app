...
class StripeSystemTest < ApplicationSystemTestCase
  setup do
    ...
    @workspace.set_payment_processor :stripe
  ...
  test "can subscribe" do
    ...
    assert @workspace.payment_processor.subscribed?
  end
  ...
  test "can update payment method" do
    ...
    assert_equal "Visa", @workspace.payment_processor.default_payment_method.brand
    assert_equal "4242", @workspace.payment_processor.default_payment_method.last4
  end
  ...
  test "can update payment method with SCA" do
    ...
    assert_selector "p", text: I18n.t("payment_methods.create.updated")
    assert_equal "Visa", @workspace.payment_processor.default_payment_method.brand
    assert_equal "3184", @workspace.payment_processor.default_payment_method.last4
  end
  ...
  test "can fail updating payment method with SCA" do
    ...
    fail_stripe_sca
    assert_selector "div", text: "We are unable to authenticate your payment method. Please choose a different payment method and try again."
    assert_nil @workspace.payment_processor.default_payment_method
  end
  ...
  test "can swap plans" do
    ...
    @workspace.set_payment_processor :stripe
    @workspace.payment_processor.payment_method_token = payment_method.id
    subscription = @workspace.payment_processor.subscribe(plan: old_plan_id)
  ...
  test "can swap plans with SCA" do
    ...
    subscription = @workspace.payment_processor.subscription
