...
class BillingAddressTest < ActionDispatch::IntegrationTest
  ...
  test "can see section for adding/updating billing address on own workspace" do
        ...
        assert_difference("Address.count", 1) do
          ...
          post billing_address_url, params: {
            address: {
              ...
              state: "TestState",
              postal_code: "12345",
              addressable: @regular_user.workspaces.first
            }
          }
        end
      end
  ...
  test "can update a billing address" do
    Jumpstart.config.stub(:payments_enabled?, true) do
      Jumpstart.config.stub(:collect_billing_address?, true) do
        ...
        assert_equal "456 Pickup Sticks Lane", @updating_address_user.personal_workspace.billing_address.line1
      end
    end
  end
end
