class Calendars::Events::YahooService < Calendars::BaseCalendarService
  ...
  def upsert_event(event)
    if event.calendar_services.find_by(provider: "yahoo").present?
      update_event(event)
    else
      insert_event(event)
    end
  ...
  def insert_event(event)
    yahoo_event_url = client.upsert_event(event)
    yahoo_event = client.get_event(yahoo_event_url)

    calendar_data = Icalendar::Event.parse(yahoo_event.calendar_data).first
    ...
    calendar_service.update!(
      ...
      etag: JSON.parse(yahoo_event.etag)
    )
    event
  end

  def update_event(event)
    yahoo_event_url = client.update_event(event)
    yahoo_event = client.get_event(yahoo_event_url)
  ...
  def delete_event(event)
    calendar_service = event.calendar_services.find_by(provider: "yahoo")
