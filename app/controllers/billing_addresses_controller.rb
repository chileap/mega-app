class BillingAddressesController < BaseController
  before_action :set_billing_address, only: [:edit, :update, :destroy]

  def new
    @billing_address = current_workspace.build_billing_address
  end

  def create
    @billing_address = current_workspace.build_billing_address(billing_address_params)
