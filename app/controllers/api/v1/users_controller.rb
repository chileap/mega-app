class Api::V1::UsersController < Api::BaseController
  ...
  def create
    ...
    # If registering with an workspace, add the WorkspaceUser with admin role
    if Jumpstart.config.register_with_workspace?
      workspace = user.owned_workspaces.first_or_initialize
      workspace.workspace_users.new(user: user, admin: true)
    end
